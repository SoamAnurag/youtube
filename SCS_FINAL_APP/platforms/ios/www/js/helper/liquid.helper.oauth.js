...
(/** @lends liquid.helper */function(helper) {
	...
	helper.oauth = {
		...
		onAuthUrlChange: function(uriLocation) {
			...
			//alert("URI Location - "+uriLocation);
			...
			if(uriLocation.indexOf("code=") != -1) {
				...
				//alert("$this.authCode SUCCESS - "+$this.authCode);
		    ...
		    else if(uriLocation.indexOf("error=") != -1) 
		    {
				...
				//alert("$this.errorMessage Error - "+$this.errorMessage);
	 ...
	 //  alert("In getRefreshToken Function - paramObj = "+ paramObj+" paramObj.auth_code - "+paramObj.auth_code); 
		   ...
		   $.ajax({
			    ...
			    .done(function(data) {
					...
					window.localStorage.setItem('accesstokenG', data.access_token);
					window.localStorage.setItem('refreshtokenG', data.refresh_token);
                     					
			    	$this.accessTokenTime = (new Date()).getTime();
			    	
                    var link="https://www.googleapis.com/oauth2/v1/userinfo?access_token="+data.access_token;
	   ...
	   //alert('In getAccessToken Function');
...
//		   console.log("Current Access Token: " + $this.accessToken);
		...
		//	   alert('Not Refreshed Old Token is valid');
	...
	//	   console.log("Getting Token from Google Server... ");
		  ...
		  // alert('alert Old Token is not valid ajax call');
		...
		//   console.log("Refresh Token >> " + refreshToken);
		   ...
		   $.ajax({
		    ...
		    .done(function(data) {
				...
				//alert('Create a new token Old token Expires');
		    	/* upon sucess, do a callback with the data received */
		    	// temporary storing access token
		    	$this.accessToken = data.access_token;
				window.localStorage.setItem('accesstokenG', data.access_token);
	  ...
	  // alert("In saveRefreshToken Function - tokenObj = "+ tokenObj);
		   ...
		   $this.getRefreshToken(tokenObj, function(data) {
			...
			//   alert(" Data - "+data.access_token);
			   ...
			   if (!data.error) {
				  ...
				  window.localStorage.setItem('accesstokenG', data.access_token);
				  //alert("Token Stored In LocalStorage - This is Refresh Token : "+ data.refresh_token);
			   }
	 ...
	 //  alert("isAuthorized Function Called");
	   
		  var $this = helper.oauth;
		  var tokenValue = window.localStorage.getItem($this.tokenKey);
		  
		//  alert("Stored TokenValue - "+tokenValue);
	 ...
	 //  alert('In getParameterByName Function');
	 //  alert('Old URL : '+url);
